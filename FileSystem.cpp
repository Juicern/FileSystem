...
class Fileos{
	public:
		...
		void cat();	//查看文件内容
		void chmod();//修改文件权限
		bool judgeMode(unsigned int mode);	//判断输入的权限是否合法
		void cls();	//清屏
		void InitNode();	//初始化目录
		void createSuper();	//创建超级用户
		void help();	//获得帮助
		void ls();	//列目录
		void login();	//登入
		void logout();	//登出
...
{
	cout<<"----------------------------------------------------------------------"<<endl;
	cout<<"-     欢迎使用RickyChu文件系统.你可以在本系统中使用下面的命令.     -"<<endl;
	cout<<"-                                cd                                  -"<<endl;
	cout<<"-                                cat                                 -"<<endl;
	cout<<"-                                chmod                               -"<<endl;
	cout<<"-                                clear                               -"<<endl;
...
void Fileos::ls()
{
	...
	while(p!=NULL)
	{
		if(p->i_flag == 1){
			cout<<p->i_name<<"  ";
            printf("%d ",&p->f);
            cout<<p->i_mode<<" "<<p->i_size<<" "<<p->i_uid<<endl;
        }
		p = p->i_next;
	}
}
...
void Fileos::chmod()
{
	...
	if(judgeMode(mode))
	{
		while(p!=NULL)
		{
			if(p->i_name == filename && p->i_flag == 1)
			{
...
void File_os()
{
	...
	while(true)
	{
		if(CURRENTUSER == rootUser->currentP->username)
			cout<<CURRENTUSER<<"(root)@surface:"<<CURRENTDIR<<"$ ";
		else
			cout<<CURRENTUSER<<"@surface:"<<CURRENTDIR<<"$ ";
		cin>>command;
		if(command == "help")
			op.help();
